...
try {
	...
	return await trainingSheet.getTraining(startDate, endDate, auth, variables)
	if (TALK_SHEET_CONST === variables)
	return await talkSheet.getTalk(startDate, endDate, auth, variables)
	throw new CustomeError("Erreur lors de la connexion au service souhaité, veuillez contacter le service support", 400)
} catch (error) {
