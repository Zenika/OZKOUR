...
async function getTalkFromDate (params, auth) {
  try {
    ...
    for (let i = 0; i <= nbMonths; i++) {
      ...
      const talkFromMonth = await sheetsWrapper.getTalks(month, year, spreadsheetId, auth)
...
const getTalk = async (start, end = dayjs(), auth, variables) => {
  try {
    ...
    return utilsArray.dateFilter(utilsCluster.verifyedCompletetData(talks, variables), formatedDateStart, formatedDateEnd)
    }
  } catch (error) {
    logger.error({message : error.message})
    throw error
